...
",
"static const double J = " * string(info["coupling constant"]) * ";\n",
"static const double omegaMin = " * string(info["omega min"]) * ";\n",
"static const double omegaMax =  " * string(info["omega max"]) * ";\n",
"static const size_t omegaPoints = " * string(info["omega points"]) * ";\n",
"static const std::complex< double > iDelta(0, " * string(info["broadening"]) * ");\n",
...
void calculate(Node *graph, std::string path)
{
    ...
    printToFile(result.format(precise), getFileName(path));
...
"
    ...
    file = open("main2.cpp", "w")
    print(file, reduce(*, contents))
    close(file)
end
