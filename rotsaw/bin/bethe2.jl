function invG(state::String)
    ...
    if !isNoint && (length(path) > 1)
        #hole magnon proximity
        Ω -= 0.5J
        #magnon magnon internactions
        Ω -= J * (length(path) - 2)
    end
...
function Sigma(state::String)
    ...
    # end
    #
    # return selfEnergy(reduce(*, moves)) ./ multiplicity
    return selfEnergy(state)
end
...
function getReachableMoves(state, excludeSelf = false)
    ...
    for d in directions
        ...
        if move == path[end-1] #if move goes back
            continue
        end
        result *= d
    end
...
function runBethe(nMagnons::Int64, J::Float64, depth::Int64 = 0)
    ## logic
end
